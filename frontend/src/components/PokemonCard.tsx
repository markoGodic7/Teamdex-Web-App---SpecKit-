...
export default function PokemonCard({ name, sprite, onOpen }: { name: string; sprite?: string | null; onOpen: (idOrName: string) => void }) {
  return (
    <div className="border rounded p-3 flex items-center gap-3">
      ...
      <button className="ml-2 px-3 py-1 border rounded" onClick={() => onOpen(name)}>Details</button>
    </div>
  );
}
